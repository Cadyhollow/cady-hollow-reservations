...
//
// The email bodies below are byte-identical to what the route sent — spliced from it, not
// retyped — so a guest's confirmation email is unchanged by this refactor.
...
export async function sendConfirmationEmails(payload: ConfirmationEmailPayload): Promise<void> {
    const {
      ...
      lateCheckout = false,
      lateCheckoutFee = 0,
      // Itemized cash lines, in order, as { label, amount } — straight from lib/pricing's
      // PricingResult.lines where the caller has it. Rendering these verbatim is what makes
      // every fee its own row instead of being absorbed into the site charge (see the
      // derived fallback below for callers that can't supply them). Label wording therefore
      // lives in lib/pricing.ts, deliberately.
      lines = [],
      nightlyRate = 0,
      // Rendered when a client has tax configured. Cady's reservations carry none today —
      // lib/pricing hardcodes taxTotal = 0 pending T3 — so these stay 0 and the row never
      // appears. Wiring reservation tax is its own task.
      taxAmount = 0,
      taxLabel = 'Sales tax',
      taxRate = 0,
    } = payload
    ...
    const row = (label: string, value: string) =>
      `<tr>
                  <td style="padding:6px 0;color:#9CA3AF;font-size:14px;">${label}</td>
                  <td style="padding:6px 0;color:#ffffff;font-size:14px;text-align:right;">${value}</td>
                </tr>`
        ...
        <body style="font-family:Arial,sans-serif;background:#f3f4f6;padding:20px;">
          <div style="max-width:500px;margin:0 auto;background:#ffffff;border-radius:12px;padding:24px;">
            ...
            <table style="width:100%;border-collapse:collapse;">
              ...
              <tr><td style="padding:6px 0;color:#6B7280;font-size:14px;">Paid</td><td style="padding:6px 0;font-size:14px;color:#166534;font-weight:bold;">$${(amountPaid / 100).toFixed(2)}</td></tr>
