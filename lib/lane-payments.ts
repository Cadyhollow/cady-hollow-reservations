 ...
 * as a missing or empty lane. A two-bucket park needs this: a "Pay both" tender settles Seasonal
 ...
 *
 * ⚠ AN EXPLICIT `lane: null` SURVIVES; A MISSING OR EMPTY LANE IS STILL DROPPED. A row that names
 * no lane because the caller never set one is malformed — dropping it is what has always happened
 * and what keeps a garbled request from being charged. A row whose lane is *explicitly* null is a
 * caller saying "this part is a whole-account payment".
 *
 * Nothing existing changes shape: every current caller sends a real lane string or no split at
 * all, so this is additive. The dropped-row fallback in the terminal route
 * (`split.length ? laneSplitTotal(split) : amount`) is why getting this wrong would be quiet
 * rather than loud — a silently dropped row would charge the card less than the screen showed.
 */
...
export async function recordCardPayment(
  ...
  const rows = split.length
    ...
    // by omitting the column, and the same thing every untagged payment on this park already is.
