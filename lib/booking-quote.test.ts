...
import { computeBookingQuote, checkDiscount, resolveNightlyRate, type QuoteInput } from './booking-quote.ts'
...
function legacyQuote(i: QuoteInput) {
  ...
  let deposit: number
  if (depositType === 'percentage') deposit = Math.min(Math.round(cashTotal * depositValue / 100), cashTotal)
  else if (depositType === 'flat') deposit = Math.min(depositValue, cashTotal)
  else if (depositType === 'full') deposit = cashTotal
