...
/**
 * NOTE — the assignee badge colour does NOT live here.
 *
 * The template re-exports `posTileColor` from this module as `personColor`. It cannot here: that
 * needs a relative import, and this repo's tsconfig does not set `allowImportingTsExtensions`, so
 * `'./pos-tiles.ts'` is a compile error (TS5097) while `'./pos-tiles'` is unresolvable to
 * `node --test`. Rather than turn a compiler flag on in a live repo, or duplicate the palette,
 * the ONE consumer (app/admin/TodoPanel.tsx) imports `posTileColor` from '@/lib/pos-tiles'
 * directly — Next resolves the alias, and this module stays import-free and therefore trivially
 * unit-testable.
 *
 * The rule that matters is unchanged: colour is hashed from the person's NAME, not their position
 * in the list, so somebody's colour does not change when a colleague is added above them — and
 * the admin has one palette, not two that drift.
 */
