...
export async function getConfirmation(reservationId: string | null): Promise<ConfirmationData | null> {
  ...
  // Named columns, not select('*'). The browser used to receive all 37 reservation columns —
  // square_payment_id, payment_method, discount_code, special_requests, waiver_signed_at and
  // `notes`, which carries the refund audit trail ("[Refund Aug 10] $220.00 refunded — …").
  // The page displays ten fields; it now receives ten.
