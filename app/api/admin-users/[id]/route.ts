...
//
// Security PR 5c-1. Owner-gated. One route for all four operations, because they share the guard
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // WHO is asking, for the self-lockout guard. A legacy shared-password session has no identity
  // (userId is null), so the "you cannot deactivate yourself" rule cannot apply to it — there is
  // no self to compare against. The last-active-Owner rule does not depend on identity and still
  // applies to every caller. 5c-2 removes that asymmetry by removing the legacy path.
