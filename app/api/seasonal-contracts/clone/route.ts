...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const toCreate: { src: any; guest: any }[] = []
    let skipped = 0
    ...
    for (const { src, guest } of toCreate) {
      ...
      if (error) {
        if ((error as any).code === '23505') { skipped++; continue } // lost the unique-constraint race → skip
        errors.push({ guest_id: guest.id, reason: error.message })
        continue
      }
      created++
    }
