...
import { computeBookingQuote, checkDiscount, resolveNightlyRate } from '@/lib/booking-quote'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // What this payment collects, by the SERVER's reading of the deposit rules.
    const serverCashToPay = paymentType === 'deposit' ? quote.deposit : quote.cashTotal
    const serverSurcharge = cardSurchargeFor(
      serverCashToPay, quote.cashTotal, quote.cashTotal,
      Number(settingsRow?.card_surcharge_percent) || 0,
    )
    ...
    const reservationPayload = {
      ...
      // every revenue report read these, so they are written from the side that is
      // authoritative rather than the side that merely asked.
      base_nightly_rate: serverNightlyRate,
      extra_guest_fee_total: quote.extraGuestFee,
      // Cash fees only: the card-only portion is deliberately kept out, matching the cash
      // total it is excluded from.
