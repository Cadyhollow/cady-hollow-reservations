...
//
// Security PR 4b. The booking page used to answer "is this code any good?" by reading the
...
//
// PR 4a already made the server the authority on whether a discount is APPLIED: /api/payment
// re-reads the row and re-runs checkDiscount() before it charges anything, so a forged code
// could not actually reduce a charge. What remained was the preview — and the read behind it.
// Now the browser asks about the one code in the box and is told about that code only.
//
// Deliberately PUBLIC and deliberately unauthenticated: campers are not logged in. That is a
// decision, not an oversight — middleware only matches /admin/:path*, so a route that wants
// auth has to call requireAdmin() itself, and this one must not. What keeps it safe is its
