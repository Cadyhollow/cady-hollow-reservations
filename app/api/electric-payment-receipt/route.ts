...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    //
    // ⚠ THE SERVER DECIDES IT, NOT THE CALLER. This figure is printed in a CAMPER'S receipt, so a
    // stale or wrong one from any caller would state a balance they do not owe.
    //
    // SEPARATED → the CAMP ACCOUNT balance. A camper who has just paid their electric must not be
    // told they still owe their season fee. COMBINED → the whole account, as always.
    //
    // ⚠ ITS OWN GUARDED SELECT for billing_mode: a park without the column must not lose its
    // receipts. Any failure lands on 'combined', which is today's behaviour here.
    ...
    if (folioId) {
      try {
        ...
        recomputed = billingMode === 'separated' ? accountBuckets(lanes).camp.balance : lanes.accountBalance
      } catch {
    ...
    const statedBalance: number = recomputed !== null ? recomputed : (Number(remainingBalance) || 0)
