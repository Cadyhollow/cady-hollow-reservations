...
// folio_payments (net of card surcharge). Mirrors the seasonals-list batch math.
//
...
export async function POST(request: NextRequest) {
  ...
  // by subtraction — the same rule accountBuckets() applies, and the reason a directory row can
  // never show a camper owing more on one card than they owe in total.
  //
  // ⚠ NO LANE CLASSIFICATION IS NEEDED, which is what keeps this a two-query endpoint. A seasonal
  // charge and a seasonal payment are both DECLARED — `lane = 'seasonal'` on the row — never
  // inferred, so no electric_readings lookup and no product_id inspection is involved.
  ...
  // `balances` is unchanged in name, shape and value — a combined park's directory reads only that
  // key and is untouched. `seasonal` is purely additive.
