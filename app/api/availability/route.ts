...
export async function GET(request: NextRequest) {
  ...
  //
  // Security PR 4b. The home page used to read `site_categories` itself with the anon key,
  // right after this route answered — a second round trip, from the browser, keyed by the ids
  // this route had just handed it. It is the same read either way, so it belongs here, where
  // the sites came from. Named columns, and scoped to the sites actually being returned.
