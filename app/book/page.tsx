...
const [cancellationPolicy, setCancellationPolicy] = useState<any>(null)
  ...
  useEffect(() => {
    if (step >= 2 && !squareLoaded) {
      loadSquare()
    }
  }, [step])
useEffect(() => {
  if (arrival) fetchCancellationPolicy()
}, [arrival])
...
async function fetchCancellationPolicy() {
    ...
    script.onload = async () => {
      try {
        const payments = (window as any).Square.payments(
          ...
          process.env.NEXT_PUBLIC_SQUARE_LOCATION_ID || 'LH42Z3KNTMZ3Q'
        )
  ...
  async function handlePayment(paymentType: 'deposit' | 'full') {
    ...
    try {
      ...
      if (result.status !== 'OK') {
        setPaymentError('Card details invalid. Please check and try again.')
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl p-6" style={{ backgroundColor: '#2B2B2B' }}>
            <h2 className="text-white font-bold text-lg mb-4">
              {step === 1 ? '1. Your Information' : '✓ Your Information'}
            </h2>
            {step === 1 ? (
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl text-white font-semibold transition-colors mt-2"
            ...
            <div className="rounded-2xl p-6" style={{ backgroundColor: '#2B2B2B' }}>
                    ...
                    <div key={addon.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-800">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="w-8 h-8 rounded-full bg-gray-700 text-white font-bold hover:bg-gray-600"
                        >-</button>
              ...
              <div className="pt-4 border-t border-gray-700 mb-6">
                ...
                <div className="flex gap-2">
                  <input
                    className="flex-1 bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm uppercase"
                ...
                {discountResult && (
                  <p className="text-green-400 text-sm mt-2">
                    ✓ {discountResult.discount_type === 'percent'
                      ...
                      : `$${(discountResult.discount_value / 100).toFixed(2)} discount applied!`}
                  </p>
                )}
              </div>

              {/* Price Summary */}
              <div className="pt-4 border-t border-gray-700 mb-6 space-y-2 text-sm">
...
<div className="rounded-lg p-4 bg-gray-800 mb-6">
  <p className="text-gray-300 text-xs leading-relaxed">
    ...
    {cancellationPolicy
      ...
      : 'Cancellations must be made at least 7 days before arrival. A 10% booking fee is retained on all cancellations.'}
  </p>
  {cancellationPolicy && !cancellationPolicy.deposit_refundable && (
    <p className="text-yellow-400 text-xs mt-2 font-medium">
      ⚠ Deposit is non-refundable for these dates.
    </p>
  )}
</div>
              ...
              <div className="mb-6">
                ...
                />
                {!squareLoaded && (
                  <p className="text-gray-400 text-sm mt-2">Loading payment form...</p>
                )}
              </div>

              {paymentError && (
                <div className="rounded-lg p-4 bg-red-900 mb-4">
          ...
          <div className="rounded-2xl p-6 sticky top-6" style={{ backgroundColor: '#2B2B2B' }}>
            ...
            <div className="space-y-3 text-sm">
              <div>
                ...
                <p className="text-white font-medium">{siteTypeLabel(site.site_type)} {site.site_number}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-white font-medium">{site.nights} night{site.nights !== 1 ? 's' : ''}</p>
              </div>
              <div className="border-t border-gray-700 pt-3">
                <p className="text-gray-400">Rate</p>
                <p className="text-white font-medium">${(site.nightly_rate / 100).toFixed(2)}/night</p>
              </div>
              <div className="border-t border-gray-700 pt-3">
