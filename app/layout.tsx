...
//
// Security PR 4c. This used to be AccentColorProvider: a client component that, on EVERY
// route including the whole admin section, opened an anon-key Supabase client and read
// settings.accent_color, then called document.documentElement.style.setProperty(). It was the
// last anon read left outside the public pages and the thing PR 6 could not revoke around.
//
// It was also redundant. The layout above already reads the settings row server-side for the
// theme, so the colour was in hand before the HTML was written — the browser was spending a
// round trip to fetch something the server had already fetched. Setting it here is the same
// mechanism the effect used (an inline style on the root element, which outranks the
// :root rule in globals.css) applied a beat earlier, so the accent is correct in the first
// paint instead of after hydration. That also removes the flash: every accent-coloured button
// and heading on / and /book rendered in the #3DBDD4 default and then changed colour.
//
// A missing, null or blank value returns undefined rather than an empty custom property, so
// the CSS default in globals.css stands — exactly what the old `if (data?.accent_color)`
// guard did.
...
export default async function RootLayout({
  ...
  // give. As of PR 4c the accent colour rides along on the same read, for the same reason.
  const settings = await getSettings();
