...
export default function MinStayPage() {
  ...
  const [editingRule, setEditingRule] = useState<MinStayRule | null>(null)
  ...
  async function fetchData() {
    const [{ data: rulesData }, { data: sitesData }] = await Promise.all([
      supabase.from('min_stay_rules').select('*').order('created_at', { ascending: false }),
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-6 py-12 text-center text-gray-400">
          No minimum stay rules yet. Click "Add Rule" to create one!
        </div>
      ) : (
