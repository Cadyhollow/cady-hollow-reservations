...
export default function GuestsPage() {
  ...
  // rather than losing the screen. This park has billing_mode but NOT the bucket-label columns,
  // so the labels fall back to the built-in names — which is the intended shape here.
  useEffect(() => {
                ...
                <div style={{ flexShrink: 0, textAlign: 'right', minWidth: 116 }}>
                  {g.id in balances ? (
                    billingMode === 'separated' && g.is_seasonal ? (
                      ...
                      // camp-tagged payments": only 15 of this park's 652 payments carry a lane,
                      // so that sum would show everyday money as still owed on almost every
                      // camper. Subtracting the seasonal half from the true account balance is
                      // exact, and the two always add back up.
                      (() => {
