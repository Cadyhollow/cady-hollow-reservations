      ...
      <Section title="Money">
        ...
        {lanes ? (
          <AccountBucketCards
            ...
            // ⚠ CADY HAS NO /admin/checkout ROUTE. The template's lane-checkout screen is not
            // part of this park's flow — payments are taken through the folio's Collect Payment
            // modal. Pointing at it would have been a dead link the moment separated mode was
            // switched on, so each card opens the folio with its bucket named instead; the doors
            // there read `?bucket=` and open on the right one.
            payHref={bucket => `/admin/folio/guest/${g.id}?bucket=${bucket}`}
        ...
        {/* ⚠ ALWAYS SHOWN — no `data.folioId` guard, and no billing-mode guard.
            ...
            WHICH SCREEN, THOUGH, DEPENDS ON THE PARK, and both are the EXISTING lane-aware path:
              separated → the folio modal, opened ON THE SEASONAL DOOR (?bucket=seasonal).
        ...
        {admin && !lanes && (
          <div className="mt-3 flex flex-wrap items-center gap-3">
            <Link href={data.billingMode === 'separated' ? `/admin/folio/guest/${g.id}?bucket=seasonal` : `/admin/folio/guest/${g.id}`}
