...
**Status:** Design locked, revised after read-only feasibility pass (Code, 2026-07-14). **Phase A built (dormant guard) — awaiting commit + live migration.** Build in phases, verify each before the next.

**Progress:**
- Phase A — BUILT, not yet committed/deployed. New: `lib/electric-periods.ts` (pure string/UTC period math: `periodFromBillingMonth`, `rangesExactMatch`, `rangesOverlap`, `overlapSpan`, `classifyPeriod`, `fmtMDY`, `billTitle`) + `lib/electric-periods.test.ts` (18/18 pass, incl. June→July 1 seam). Modified `app/admin/electric-billing/page.tsx` (added `billGuard` + `prepareBill()` resolver on the single-row action, red/amber/blue confirm box; `sendBill` + `billing_month` logic untouched). Three review-only SQL scripts in `db/migrations/` (two ALTERs + backfill — NOT run). Guard is dormant until Phase E (see Phase A note). `tsc --noEmit` exit 0, `next build` exit 0.
...
- **Test account (`bc338372…`, site C2, your own):** confirmed messy in exactly the useful way — **May 2026 billed twice** (2 readings + 2 line items, $29.97 and $56.70 → an exact-range duplicate) and a **November orphan** (reading for $119.98 whose charge was hard-deleted → the wedge, in real data). These will make the guard fire the moment it ships — which is the intended first test, not a problem.
- **Balance-summation sites: 10 total** need the `voided = false` filter (see Decision 2). `lib/ledger.ts` (powers the emailed statement) is the critical one and has no void concept at all.
...
- **Phase C — Void mechanism (Decision 2).** RPC to void both halves atomically; `voided_at`/`voided_by`/`reason`. Make the `folio/[id]:413` filter real. Verify on the test account: void the May duplicate and the Nov orphan, confirm you can then re-bill those periods.
- **Phase D — Balance/statement void-filter.** Apply `WHERE voided = false` consistently (centralized) across all 10 balance-summation sites (`lib/ledger.ts` is critical — powers the emailed statement, has no void concept today). Verify a voided bill disappears from customer balance everywhere.
- **Phase E — Page shape + prevention UI.** Readable list (site-ordered) + name/site search; replace the month dropdown + "✓ Billed" sent-gate with flexible period entry; "Bill Electric" gated on period confirm (pre-filled to previous); send-time confirmation naming the range; **route bulk send through the guard too.** This is where the guard becomes live and demonstrable.
