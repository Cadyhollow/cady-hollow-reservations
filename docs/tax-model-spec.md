...
5. **Onboarding SQL** (`resonation-admin/app/api/onboard/route.ts` `DATABASE_SETUP_SQL`) must include the new tables, or new clients provision without them — the "repo lies about the database" trap that has bitten before.
...
- `/admin/manual-booking` is unlinked but **URL-reachable** and sends `surchargeAmount: 0`. Delete the page.
...
**Propagation** (template + Lakeshore + onboarding SQL) after Cady validates — this one carries **schema + seed data**, not just code.
