...
//
// PR 5b-2 added --role, for one reason: role enforcement cannot be TESTED with only an Owner
...
//
//   node scripts/seed-user.mjs manager@cadyhollow.test "Test Manager" --role manager
//   node scripts/seed-user.mjs staff@cadyhollow.test   "Test Staff"   --role staff
//
...
//
// Public signup is OFF at the Supabase project level (verified on Cady: /auth/v1/settings reports
// disable_signup: true), so this is the ONLY way an account comes into existence until 5c ships
// the Owner's account-management screen. Each newly provisioned client project starts with signup
// ON and needs the same treatment — that is a PR 7 item.
//
