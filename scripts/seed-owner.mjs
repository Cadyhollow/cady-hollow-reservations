// Create the FIRST Owner account. One-time bootstrap, run by hand from a laptop.
//
//   node scripts/seed-owner.mjs <email> [full name]
//
...
//
// Public signup stays OFF at the Supabase project level, so this is the ONLY way an account comes
// into existence until 5c ships the Owner's account-management screen (itself owner-gated and
// server-side).
//
...
if (!email || !email.includes('@')) {
  console.error('Usage: node scripts/seed-owner.mjs <email> [full name]')
  process.exit(1)
}
...
const { error: profileError } = await admin
  .from('profiles')
  .upsert({ id: userId, email, full_name: fullName, role: 'owner', active: true }, { onConflict: 'id' })
...
console.log(`\nOwner ready: ${email} (${userId}) role=owner active=true`)
